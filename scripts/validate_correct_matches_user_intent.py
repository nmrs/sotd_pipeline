...
class CorrectMatchesValidator:
    ...
    def load_correct_matches(self) -> Dict[str, Any]:
        ...
        if not self.correct_matches_path.exists():
            raise FileNotFoundError(
                f"Correct matches file not found: {self.correct_matches_path}"
            )
            
        with open(self.correct_matches_path, 'r', encoding='utf-8') as f:
            return yaml.safe_load(f)
    
    def extract_brush_entries(self, correct_matches: Dict[str, Any]) -> List[Tuple[str, str, Dict[str, Any]]]:
        """
    ...
    def validate_brush_entry(self, entry_type: str, brush_string: str, entry_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def enhance_correct_matches(self, correct_matches: Dict[str, Any], validation_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def generate_validation_report(self, validation_results: List[Dict[str, Any]], enhanced_entries: List[str]) -> str:
        """
...
"""
            ...
            for result in validation_results:
                if result["status"] == "failed":
                    ...
                    if "match_result" in result:
                        report += f"- Match result: {json.dumps(result['match_result'], indent=2)}\n"
                    report += "\n"
    ...
    def save_enhanced_correct_matches(self, enhanced_matches: Dict[str, Any], backup: bool = True) -> None:
        """
    ...
    def run_validation(self, dry_run: bool = False) -> None:
        ...
        enhanced_matches, enhanced_entries = self.enhance_correct_matches(correct_matches, validation_results)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate and enhance correct_matches.yaml with user intent")
    parser.add_argument("--dry-run", action="store_true", help="Don't save changes, just generate report")
