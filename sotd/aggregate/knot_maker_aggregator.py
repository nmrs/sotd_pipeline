#!/usr/bin/env python3
"""Brush knot maker aggregation using the base aggregator pattern."""
...
class KnotMakerAggregator(BaseAggregator):
    """Aggregator for brush knot maker usage statistics."""

    def get_operation_name(self) -> str:
        """Return the operation name for monitoring."""
        return "aggregate_brush_knot_makers"

    def get_product_type(self) -> str:
        """Return the product type being aggregated."""
        return "brush_knot_maker"
    ...
    def _extract_from_record(
        ...
        """
        Extract brush knot maker data from a single record.
        ...
        Returns:
            Extracted knot maker data dictionary or None if invalid
        """
        # Extract brush info
        ...
        # Extract knot maker info
        knot_maker = matched.get("knot_maker")
        if not knot_maker:
            return None
...
def aggregate_brush_knot_makers(
    ...
    """
    Aggregate brush knot maker usage statistics using the new base aggregator pattern.
    ...
    Returns:
        List of brush knot maker aggregation results

    Raises:
        ValueError: If records list is invalid or contains invalid data
    """
    aggregator = KnotMakerAggregator(debug)
