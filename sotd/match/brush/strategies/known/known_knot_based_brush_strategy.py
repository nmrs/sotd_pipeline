...
class KnownKnotBasedBrushMatchingStrategy(BaseBrushMatchingStrategy):
    """
    ...
    def match(self, value: str) -> MatchResult:
        """
        Common pattern matching logic for known knot-based brush strategies.
    ...
    def _create_match_result_from_pattern(self, value: str, pattern_data: dict) -> dict:
        ...
        knot_brand = pattern_data.get("knot_brand", brand)  # Use knot brand if specified, otherwise brush brand
        knot_model = pattern_data.get("knot_model", model)  # Use knot model if specified, otherwise brush model
        knot_fiber = pattern_data.get("knot_fiber") or fiber_info.get("fiber")  # Use knot fiber if specified
        knot_size = pattern_data.get("knot_size_mm") or size_info.get("knot_size_mm")  # Use knot size if specified

        # Handle nested handle structure if present
        handle_brand = pattern_data.get("handle_brand", brand)  # Use handle brand if specified, otherwise brush brand
    ...
    def _create_match_result(
        ...
        return create_match_result(
            ...
            strategy="known_knot_based_brush",
        )
