...
def is_blade_matched(record: dict) -> bool:
    return (
        ...
        and bool(record["blade"]["matched"].get("blade"))
    )


def match_record(record: dict, razor_matcher: RazorMatcher, blade_matcher: BladeMatcher) -> dict:
    result = record.copy()
    if "razor" in result:
...
def run_match(args):
    ...
    for year, month in tqdm(_month_span(args), desc="Months", unit="month"):
        ...
        processed = [
            match_record(record, razor_matcher, blade_matcher)
        ...
        no_matches = sum(1 for r in processed if not (is_razor_matched(r) or is_blade_matched(r)))
