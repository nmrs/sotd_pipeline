...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Analyze unmatched razors for pattern and candidate discovery"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Maximum number of unmatched razors to show (default: 20)",
    )
    parser.add_argument(
        "--field",
        type=str,
    ...
    print(
        f"\n🔍 Found {len(all_unmatched)} unique unmatched {args.field} descriptions across all files:\n"
    )
