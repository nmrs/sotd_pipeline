...
class BaseTableGenerator(ABC):
    ...
    def _calculate_column_widths(
        ...
        """Calculate optimal column widths based on content and configuration.
        ...
        for col in df.columns:
            ...
            # Calculate width based on content and configuration
            max_width = 0
            ...
            # Apply minimum widths for specific formats
            if config.get("format") == "number":
                max_width = max(max_width, 8)  # Minimum width for numbers
            elif config.get("format") == "decimal":
                max_width = max(max_width, 10)  # Minimum width for decimals
            elif config.get("format") == "delta":
    ...
    def _format_table_row(
        self, row_values: list, column_widths: Dict[str, int], df: pd.DataFrame
    ) -> str:
        """Format a table row with proper column spacing.

        Args:
            row_values: List of cell values for the row
            column_widths: Dictionary mapping column names to widths
            df: DataFrame to get column names

        Returns:
            Formatted row string with proper spacing
        """
        formatted_cells = []
        for i, cell_value in enumerate(row_values):
            ...
            if col_name in column_widths:
                ...
                if col_name in ["shaves", "unique users", "avg shaves per user"]:
                    # Right-align numeric columns
                    formatted_cell = cell_str.rjust(col_width)
                else:
                    # Left-align text columns
                    formatted_cell = cell_str.ljust(col_width)
            else:
    ...
    def _format_table_header(self, df: pd.DataFrame, column_widths: Dict[str, int]) -> str:
        """Format table header with proper column spacing.
        ...
        Returns:
            Formatted header string with proper spacing
        """
        formatted_headers = []
        for col in df.columns:
            col_width = column_widths.get(col, len(col))
            # Left-align headers
            formatted_header = col.ljust(col_width)
    ...
    def _format_table_separator(
        ...
        """Format table separator with proper column spacing and alignment.
        ...
        Returns:
            Formatted separator string with proper spacing and alignment
        """
        separator_parts = []
        ...
        for col in df.columns:
            ...
            # Get column configuration
            config = column_config.get(orig_col, {}) if orig_col else {}
            col_width = column_widths.get(col, 3)

            # Determine alignment and format separator
            if config.get("format") == "delta":
                # Center-aligned for deltas
                separator_part = ":" + "-" * (col_width - 2) + ":"
            elif col in ["shaves", "unique users", "avg shaves per user"]:
                # Right-aligned for numeric columns
                separator_part = "-" * (col_width - 1) + ":"
            else:
                # Left-aligned for text columns
                separator_part = "-" * col_width

            separator_parts.append(separator_part)
