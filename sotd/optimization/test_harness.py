...
class BrushMatchingTestHarness:
    ...
    def _test_configuration(
        ...
        try:
            ...
            results["overall_success_rate"] = (
                results["successful_matches"] / results["total_tests"]
            )
    ...
    def _test_single_case(
        ...
        try:
            ...
            validation_result = self._validate_test_case_result(
                result.matched, test_case
            )
    ...
    def _validate_composite_brush(
        ...
        if "expected_handle_brand" in test_case:
            ...
            elif (handle_brand.lower() != expected_handle_brand.lower() or
                  handle_model.lower() != expected_handle_model.lower()):
                handle_validation = {
                    "success": False,
                    "partial_match": False,
        ...
        if "expected_knot_brand" in test_case:
            ...
            elif (knot_brand.lower() != expected_knot_brand.lower() or
                  knot_model.lower() != expected_knot_model.lower()):
                knot_validation = {
                    "success": False,
                    "partial_match": False,
    ...
    def _validate_handle_only(
        ...
        if (handle_brand.lower() == expected_handle_brand.lower() and
            handle_model.lower() == expected_handle_model.lower()):
            return {"success": True, "partial_match": False}
        else:
            return {
    ...
    def _validate_knot_only(
        ...
        if (knot_brand.lower() == expected_knot_brand.lower() and
            knot_model.lower() == expected_knot_model.lower()):
            return {"success": True, "partial_match": False}
        else:
            return {
    ...
    def _calculate_improvement_metrics(
        ...
        relative_improvement = (
            ...
            if original_success_rate > 0 else float("inf")
        )
    ...
    def _analyze_regressions(
        ...
        for i, (orig_result, opt_result) in enumerate(zip(
            original_results["detailed_results"],
            optimized_results["detailed_results"]
        )):
    ...
    def generate_validation_report(self) -> str:
        ...
        for category, metrics in self.validation_metrics["improvement"]["category_improvements"].items():
        ...
        for difficulty, metrics in self.validation_metrics["improvement"]["difficulty_improvements"].items():
