...
class BrushConfigurationUpdater:
    ...
    def apply_weight_adjustments(self, adjustments: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if any_invalid and len([w for w in weight_updates.values() if not self._validate_weight_value(w)]) == len(weight_updates):
    ...
    def apply_modifier_adjustments(self, adjustments: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "brush_scoring_weights" not in config or "strategy_modifiers" not in config["brush_scoring_weights"]:
            ...
            for modifier_name, adjustment_value in modifier_updates.items():
                ...
                if not isinstance(adjustment_value, (int, float)):
                    logger.warning(f"Invalid modifier adjustment value for {modifier_name}: {adjustment_value}")
                    continue
                ...
                for strategy_name, modifiers in strategy_modifiers.items():
                    if modifier_name in modifiers:
                        ...
                        logger.info(f"Updated {strategy_name}.{modifier_name} to {adjustment_value}")
                ...
                for strategy_name, weight in suggested_weights.items():
                    if strategy_name in strategy_modifiers:
                        ...
                        if isinstance(weight, (int, float)):
                            ...
                            logger.info(f"Added new modifier {modifier_name} to {strategy_name} with weight {weight}")
                        else:
                            logger.warning(f"Invalid weight for {modifier_name} in {strategy_name}: {weight}")
                    else:
                        logger.warning(f"Unknown strategy {strategy_name} for modifier {modifier_name}")
    ...
    def preview_changes(self, adjustments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "modifier_adjustments" in adjustments:
                strategy_modifiers = config.get("brush_scoring_weights", {}).get("strategy_modifiers", {})
