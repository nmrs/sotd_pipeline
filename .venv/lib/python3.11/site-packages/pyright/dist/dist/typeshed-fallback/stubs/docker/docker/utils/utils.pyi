...
@deprecated(
    "utils.create_ipam_pool has been removed. Please use a docker.types.IPAMPool object instead."
)
def create_ipam_pool(*args, **kwargs) -> NoReturn: ...
@deprecated(
    "utils.create_ipam_config has been removed. Please use a docker.types.IPAMConfig object instead."
)
...
def convert_service_networks(
    networks: list[str] | list[dict[str, str]] | list[str | dict[str, str]],
) -> list[dict[str, str]]: ...
def parse_repository_tag(repo_name: str) -> tuple[str, str | None]: ...
@overload
def parse_host(
    addr: None, is_win32: Literal[True], tls: bool = False
) -> Literal["npipe:////./pipe/docker_engine"]: ...
@overload
def parse_host(
...
def parse_devices(
    devices: Iterable[str | dict[str, Incomplete]],
...
def format_extra_hosts(
    extra_hosts: Mapping[object, object],
    task: bool = False,  # keys and values are converted to str
) -> list[str]: ...
@deprecated(
    "utils.create_host_config has been removed. Please use a docker.types.HostConfig object instead."
)
