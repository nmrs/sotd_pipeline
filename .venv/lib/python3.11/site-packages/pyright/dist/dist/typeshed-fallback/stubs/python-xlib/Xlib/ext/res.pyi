...
def query_version(
    self: Display | resource.Resource, client_major: int = 1, client_minor: int = 2
) -> QueryVersion: ...
...
def query_client_resources(
    self: Display | resource.Resource, client: int
) -> QueryClientResources: ...

class QueryClientPixmapBytes(rq.ReplyRequest): ...

def query_client_pixmap_bytes(
    self: Display | resource.Resource, client: int
...
class SizeOf(rq.LengthOf):
    item_size: int
    def __init__(
        self, name: str | list[str] | tuple[str, ...], size: int, item_size: int
    ) -> None: ...
...
def query_client_ids(
    self: Display | resource.Resource, specs: Sequence[tuple[int, int]]
) -> QueryClientIds: ...
