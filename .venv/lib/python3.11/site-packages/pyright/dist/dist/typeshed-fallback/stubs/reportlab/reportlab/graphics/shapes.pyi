...
class Drawing(Group, Flowable):
    ...
    def __init__(
        ...
        width: float = 400,
        height: float = 200,
        *nodes: Shape | UserNode,
        **keywords: Unpack[_DrawingKwArgs],
    ) -> None: ...
    ...
    def resized(
        self,
        kind: Literal["fit", "fitx", "fity"] = "fit",
        lpad: float = 0,
        rpad: float = 0,
        bpad: float = 0,
        tpad: float = 0,
...
class Line(LineShape):
    ...
    y2: float
    def __init__(
        self, x1: float, y1: float, x2: float, y2: float, **kw: Unpack[_LineShapeKwArgs]
    ) -> None: ...
...
def definePath(
    pathSegs: Iterable[_PathOp] = [],
    isClipPath: _BoolLike = 0,
    dx: float = 0,
    dy: float = 0,
    **kw: Unpack[_DefinePathKwArgs],
) -> Path: ...
...
class Image(SolidShape):
    ...
    path: Incomplete
    def __init__(
        self, x: float, y: float, width: float, height: float, path, **kw: Unpack[_SolidShapeKwArgs]
    ) -> None: ...
    def copy(self) -> Self: ...
    def getBounds(self) -> tuple[float, float, float, float]: ...
...
class Ellipse(SolidShape):
    ...
    ry: float
    def __init__(
        self, cx: float, cy: float, rx: float, ry: float, **kw: Unpack[_SolidShapeKwArgs]
    ) -> None: ...
    def copy(self) -> Self: ...
    def getBounds(self) -> tuple[float, float, float, float]: ...
