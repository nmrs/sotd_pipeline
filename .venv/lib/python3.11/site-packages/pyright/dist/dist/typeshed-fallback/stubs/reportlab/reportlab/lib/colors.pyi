...
_ConvertibleToColor: TypeAlias = (
    Color | list[float] | tuple[float, float, float, float] | tuple[float, float, float] | str | int
)
...
class Color:
    ...
    alpha: float
    def __init__(
        self, red: float = 0, green: float = 0, blue: float = 0, alpha: float = 1
    ) -> None: ...
    @property
...
def cmyk2rgb(
    cmyk: tuple[float, float, float, float], density: float = 1
...
def linearlyInterpolatedColor(
    c0: _ColorT, c1: _ColorT, x0: float, x1: float, x: float
) -> _ColorT: ...
@overload
def obj_R_G_B(
