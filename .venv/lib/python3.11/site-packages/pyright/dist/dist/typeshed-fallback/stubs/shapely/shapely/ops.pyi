...
class CollectionOperator:
    ...
    def linemerge(
        self,
        lines: MultiLineString | BaseMultipartGeometry | Iterable[_ConvertibleToLineString],
        directed: bool = False,
...
def triangulate(
    geom: Geometry, tolerance: float = 0.0, edges: Literal[False] = False
) -> list[Polygon]: ...
@overload  # edges true (keyword)
def triangulate(
    geom: Geometry, tolerance: float = 0.0, *, edges: Literal[True]
) -> list[LineString]: ...
@overload  # edges true (positional)
def triangulate(geom: Geometry, tolerance: float, edges: Literal[True]) -> list[LineString]: ...
@overload  # fallback
def triangulate(
    geom: Geometry, tolerance: float = 0.0, edges: bool = False
...
def transform(
    func: Callable[[float, float, float | None], tuple[float, ...]], geom: GeoT
) -> GeoT: ...
...
def clip_by_rect(
    geom: Geometry, xmin: float, ymin: float, xmax: float, ymax: float
) -> BaseGeometry: ...
