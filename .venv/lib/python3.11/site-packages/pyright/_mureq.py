...
def request(method, url, *, read_limit=None, **kwargs):
    ...
    with yield_response(method, url, **kwargs) as response:
        ...
        return Response(
            response.url, response.status, _prepare_incoming_headers(response.headers), body
        )
    ...
    *,
    ...
    if is_unix:
        ...
        conn = HTTPSConnection(
            host, port, source_address=source_address, timeout=timeout, context=ssl_context
        )
    else:
