        ...
        if ireq.is_direct:
            if isinstance(download_info.info, VcsInfo):
                ...
                package.directory = PackageDirectory(
                    ...
                    editable=(download_info.info.editable if download_info.info.editable else None),
                    subdirectory=download_info.subdirectory,
                )
...
class Pylock:
    ...
    def from_install_requirements(
        ...
        return cls(
            packages=sorted(
                (Package.from_install_requirement(ireq, base_dir) for ireq in install_requirements),
                key=lambda p: p.name,
            )
        )
