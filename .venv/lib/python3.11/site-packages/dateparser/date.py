...
def get_date_from_timestamp(date_string, settings, negative=False):
    ...
    if match:
        if settings is None or settings.TIMEZONE is None or "local" in settings.TIMEZONE.lower():
...
class DateDataParser:
    ...
    def __init__(
        ...
        if not isinstance(use_given_order, bool):
            raise TypeError(
                "use_given_order argument must be a boolean (%r given)" % type(use_given_order)
            )
    ...
    def _get_applicable_locales(self, date_string):
        ...
        def date_strings():
            ...
            yield date_string
            if not pop_tz_cache:
                stripped_date_string, _ = pop_tz_offset_from_string(date_string, as_offset=False)
