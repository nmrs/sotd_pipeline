...
class Locale:
    ...
    def _get_relative_translations(self, settings=None):
        if settings.NORMALIZE:
            if self._normalized_relative_translations is None:
                self._normalized_relative_translations = self._generate_relative_translations(
                    normalize=True
                )
    ...
    def _split(self, date_string, keep_formatting, settings=None):
        ...
        tokens = list(self._split_tokens_by_known_words(tokens, keep_formatting, settings=settings))
        return tokens
