...
class Dictionary:
    ...
    def __init__(self, locale_info, settings=None):
        ...
        dictionary.update(
            zip_longest(map(methodcaller("lower"), PARSER_KNOWN_TOKENS), PARSER_KNOWN_TOKENS)
        )
    ...
    def _split_by_known_words(self, string: str, keep_formatting: bool):
        ...
        while unknown:
            ...
            if not match:
                ...
                curr_split = [known] if self._should_capture(known, keep_formatting) else []
                if unparsed and self._should_capture(unparsed, keep_formatting):
                    curr_split = self._split_by_numerals(unparsed, keep_formatting) + curr_split
                if unknown:
    ...
    def _get_sorted_relative_strings_from_cache(self):
        if (
            ...
            self._add_to_cache(
                ...
                value=sorted(
                    [PARENTHESES_PATTERN.sub("", key) for key in self._relative_strings],
                    key=len,
                    reverse=True,
                ),
            )
