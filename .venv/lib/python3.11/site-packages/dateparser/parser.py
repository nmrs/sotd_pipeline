...
class _parser:
    ...
    def __init__(self, tokens, settings):
        ...
        self.ordered_num_directives = OrderedDict(
            (k, self.num_directives[k]) for k in (resolve_date_order(settings.DATE_ORDER, lst=True))
        )
        ...
        for index, token_type_original_index in enumerate(self.filtered_tokens):
            ...
            if self.time is None:
                ...
                try:
                    ...
                    _is_after_period = (
                        original_index != 0 and self.tokens[original_index - 1][0] == "."
                    )
    ...
    def _results(self):
        missing = [field for field in ("day", "month", "year") if not getattr(self, field)]
    ...
    def _correct_for_day(self, dateobj):
        ...
        dateobj = set_correct_day_from_settings(dateobj, self.settings, current_day=self.now.day)
        return dateobj
