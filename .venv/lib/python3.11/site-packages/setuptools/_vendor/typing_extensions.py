    ...
    class _Final(typing._FinalTypingBase, _root=True):
        ...
        def __getitem__(self, item):
            ...
            if self.__type__ is None:
                return cls(
                    typing._type_check(item, f"{cls.__name__[1:]} accepts only single type."),
                    _root=True,
                )
        ...
        def __init_subclass__(cls, *args, **kwargs):
            ...
            if "__orig_bases__" in cls.__dict__:
                ...
                if gvars is None:
                    ...
                    if not tvarset <= gvarset:
                        ...
                        raise TypeError(
                            f"Some type variables ({s_vars}) are not listed in {the_base}[{s_args}]"
                        )
                    tvars = gvars
            ...
            for base in cls.__bases__:
                if not (
                    ...
                    raise TypeError(
                        f"Protocols can only inherit from other protocols, got {repr(base)}"
                    )
...
# 3.6
    ...
    class _ProtocolMeta(GenericMeta):
        ...
        def __new__(
            ...
            if tvars is not None:
                ...
                for base in bases:
                    ...
                    if isinstance(base, GenericMeta) and base.__origin__ in (
                        ...
                        if gvars is not None:
                            raise TypeError(
                                "Cannot inherit from Generic[...] or Protocol[...] multiple times."
                            )
                        gvars = base.__parameters__
                if gvars is None:
                    gvars = tvars
                    ...
                    if not tvarset <= gvarset:
                        ...
                        raise TypeError(
                            f"Some type variables ({s_vars}) are not listed in {cls_name}[{s_args}]"
                        )
                    tvars = gvars
            ...
            self.__args__ = (
                tuple(
                    ... if a is typing._TypingEllipsis else () if a is typing._TypingEmpty else a
                    for a in args
                )
                if args
                else None
...
# 3.7-3.9
    ...
    def get_origin(tp):
        ...
        if isinstance(
            tp,
            (typing._GenericAlias, GenericAlias, _BaseGenericAlias, ParamSpecArgs, ParamSpecKwargs),
        ):
...
# 3.6
    ...
    class _TypeGuard(typing._FinalTypingBase, _root=True):
        ...
        def __getitem__(self, item):
            ...
            if self.__type__ is None:
                return cls(
                    typing._type_check(item, f"{cls.__name__[1:]} accepts only a single type."),
                    _root=True,
                )
            raise TypeError(f"{cls.__name__[1:]} cannot be further subscripted")
