...
def parse_configuration(
    distribution: "Distribution", command_options: AllCommandOptions, ignore_option_errors=False
    ...
    def _exclude_files_parser(cls, key):
        ...
        def parser(value):
            ...
            if value.startswith(exclude_directive):
                raise ValueError(
                    "Only strings are accepted for the {0} field, files are not accepted".format(
                        key
                    )
                )
            return value
...
class ConfigOptionsHandler(ConfigHandler["Distribution"]):
    ...
    def parse_section_extras_require(self, section_options):
        ...
        parsed = self._parse_section_to_dict_with_key(
            section_options, lambda k, v: self._parse_requirements_list(f"extras_require[{k}]", v)
        )
