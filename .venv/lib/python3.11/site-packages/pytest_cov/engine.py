...
class CovController:
    ...
    def __init__(
        self, options: argparse.Namespace, config: Union[None, object], nodeid: Union[None, str]
    ):
    ...
    def summary(self, stream):
        ...
        if any(x in self.cov_report for x in ["term", "term-missing"]):
            ...
            skip_covered = (
                isinstance(self.cov_report, dict) and "skip-covered" in self.cov_report.values()
            )
        ...
        if "html" in self.cov_report:
            ...
            stream.write(
                f"Coverage HTML written to dir {self.cov.config.html_dir if output is None else output}\n"
            )
        ...
        if "xml" in self.cov_report:
            ...
            stream.write(
                f"Coverage XML written to file {self.cov.config.xml_output if output is None else output}\n"
            )
        ...
        if "json" in self.cov_report:
            ...
            stream.write(
                "Coverage JSON written to file %s\n"
        ...
        if "lcov" in self.cov_report:
            ...
            stream.write(
                f"Coverage LCOV written to file {self.cov.config.lcov_output if output is None else output}\n"
            )
    ...
    def start(self):
        ...
        if not self.is_collocated:
            ...
            if self.cov_source is not None:
                self.cov_source = [
                    source.replace(master_topdir, worker_topdir) for source in self.cov_source
                ]
