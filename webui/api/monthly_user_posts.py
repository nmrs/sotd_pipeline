...
async def get_users_for_month(
    month: str,
    search: Optional[str] = Query(None, description="Search filter for usernames")
) -> List[Dict[str, Any]]:
...
async def get_user_posting_analysis(
    month: str,
    username: str
) -> UserPostingAnalysis:
    ...
    try:
        ...
        if not enriched_data:
            raise HTTPException(
                status_code=404, 
                detail=f"No data available for {month}"
            )
        ...
        analysis["posted_dates"] = [
            d.isoformat() for d in analysis["posted_dates"]
        ]
