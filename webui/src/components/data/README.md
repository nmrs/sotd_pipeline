...
const MyComponent = () => {
  const brushSplits = [{ original: 'Test Brush', handle: 'Test Handle', knot: 'Test Knot' }];
...
const MyComponent = () => {
  ...
  return (
    <VirtualizedTable data={data} columns={columns} height={400} rowHeight={48} resizable={true} />
  );
};
```
