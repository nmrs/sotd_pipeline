...
**⚠️ CRITICAL WARNING: FILE SIZE AND RECORD COUNT COMPARISON ARE INSUFFICIENT**
...
### **STEP 3: MANDATORY Pipeline Re-Run** ✅
...
### **STEP 4: MANDATORY Backup Post-Fix Outputs** ✅
...
### **STEP 5: MANDATORY Baseline Comparison** ✅
   - **⚠️ CRITICAL: File size and record count comparison are INSUFFICIENTLY DETERMINISTIC**
   - **REQUIRED: Full JSON content comparison (ignoring `"metadata"` sections only)**
   - **⚠️ MANDATORY: You MUST compare ALL 12 files, not just the ones you suspect may have changed**
   - **⚠️ MANDATORY: You MUST compare the actual content, not just counts or file sizes**
   ...
   - **⚠️ CRITICAL: You cannot assume which files will be affected by your changes**
   - **⚠️ CRITICAL: Pipeline phases are interconnected - changes in one phase can affect others**
   - **⚠️ CRITICAL: Always compare ALL files to catch unexpected side effects**

   **⚠️ MANDATORY: Comprehensive File Validation (ALL 12 files)**:
   ...
   **⚠️ CRITICAL: Why ALL files matter:**
- **Pipeline phases are interconnected** - changes in fetch can affect extract, match, enrich, aggregate, report
...
- **Fetch changes cascade through everything** - thread fetching affects all downstream data processing
- **You cannot predict side effects** - always validate everything to catch unexpected changes

### **STEP 6: Branch Based on Comparison Results** ✅
...
- [ ] **STEP 1**: Task annotated with start_hash ✅
- [ ] **STEP 2**: Fix implemented and tests pass ✅  
- [ ] **STEP 3**: Pipeline re-run completed successfully ✅
- [ ] **STEP 4**: Post-fix outputs backed up ✅
- [ ] **STEP 5**: Baseline comparison shows identical content ✅
- [ ] **STEP 6**: Task marked complete in TEST_SUITE_TODO.md ✅

**ONLY when ALL 6 steps are confirmed can you proceed to the next task.**

## **🔍 VALIDATION REQUIREMENTS**

**⚠️ MANDATORY: You MUST validate ALL 12 files, not just some of them**

**What you CANNOT do:**
- ❌ Validate only files you think might have changed
- ❌ Skip validation of certain phases
- ❌ Only check file sizes or record counts
- ❌ Assume changes in one phase won't affect others
- ❌ Validate only JSON files and skip markdown reports

**What you MUST do:**
- ✅ Validate ALL 12 files (8 JSON + 4 MD)
- ✅ Compare actual content, not just metadata
- ✅ Use proper JSON parsing tools for comparison
- ✅ Check for both regressions AND improvements
- ✅ Document any differences found and their impact
- ✅ Only proceed if ALL files are identical (outside metadata)

**⚠️ CRITICAL: Pipeline phases are interconnected**
- **Fetch** changes can affect **Extract**, **Match**, **Enrich**, **Aggregate**, **Report** phases
- **Extract** changes can affect **Match**, **Enrich**, **Aggregate**, **Report** phases
- **Match** changes can affect **Enrich**, **Aggregate**, **Report** phases
- **Enrich** changes can affect **Aggregate**, **Report** phases
- **Aggregate** changes can affect **Report** generation
- **Any phase** can affect downstream phases and final outputs
...
**⚠️ CRITICAL: You cannot skip validation of any file**
**⚠️ CRITICAL: You cannot assume which files will be affected**
**⚠️ CRITICAL: Always validate everything to catch unexpected side effects**
