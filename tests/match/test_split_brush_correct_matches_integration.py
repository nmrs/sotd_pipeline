...
class TestSplitBrushCorrectMatchesIntegration:
    ...
    def test_brush_matcher_case_insensitive_matching(
        self, config, split_brush_correct_matches, tmp_path
    ):
        ...
        for test_input in test_cases:
            ...
            assert (
                result.pattern == "correct_matches_split_brush"
            ), f"Should be split brush correct match for: {test_input}"
    ...
    def test_split_brush_priority_over_other_matching(
        self, config, split_brush_correct_matches, tmp_path
    ):
        ...
        ), "Should prioritize correct matches over split detection"
        ...
        assert (
            handle["_matched_by"] == "CorrectMatches"
        ), "Should use correct matches, not HandleMatcher"
        assert (
            knot["_matched_by"] == "CorrectMatches"
        ), "Should use correct matches, not BrushSplitter"
    ...
    def test_split_brush_not_found_falls_back_to_split_detection(
        self, config, split_brush_correct_matches, tmp_path
    ):
