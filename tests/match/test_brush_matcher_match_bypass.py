...
class TestBrushMatcherMatchBypass:
    ...
    def test_match_method_should_accept_bypass_parameter(self):
        ...
        try:
            ...
            if hasattr(result2, 'matched') and result2.matched:
                ...
                assert strategy_used != 'correct_matches', f"Bypass should not use correct_matches strategy, got: {strategy_used}"
                
        except TypeError as e:
            # This is the bug - the method doesn't accept the parameter
            print(f"🔍 DEBUG: Bug exposed! Method doesn't accept bypass_correct_matches parameter: {e}")
    ...
    def test_current_workaround_creates_bug(self):
        ...
        print(f"🔍 DEBUG: Matcher bypass_correct_matches attribute: {getattr(bypass_matcher, 'bypass_correct_matches', 'NOT_FOUND')}")
