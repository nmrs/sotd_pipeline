...
class TestSoapSampleAggregationIntegration:
    ...
    def test_soap_sample_aggregation_in_processor(self):
        ...
        assert "sample - B&M - Seville" in sample_names, "B&M Seville sample should be aggregated"
        assert (
            "tester - Declaration Grooming - Persephone" in sample_names
        ), "Declaration Grooming tester should be aggregated"

        # Check that regular soaps are not included
        assert (
            "Stirling - Bay Rum" not in sample_names
    ...
    def test_soap_sample_aggregation_empty_data(self):
        ...
        assert (
            result["data"]["soap_samples"] == []
        ), "soap_samples should be empty list when no samples"
